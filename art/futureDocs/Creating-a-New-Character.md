...
At the top of the file you need add the class to the character package with `package characters;`. Then you need to define the class with `class {ClassName} extends CharacterInfoBase`. Inside the class you need to create a constructor with `public function new()` where you define the character info. Don't forget to call `super()`.
