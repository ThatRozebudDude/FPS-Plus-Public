...
Additionally, scripted classes cannot have the same class name if they are in the same package. This means you cannot have 2 characters that are named `Boyfriend` but you can have a character named `Boyfriend` and a Freeplay DJ named `Boyfriend` as they are in different packages.
