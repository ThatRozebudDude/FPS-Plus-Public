...
At the top of the file you need add the class to the stage package with `package stages;`. Then you need to define the class with `class {ClassName} extends BaseStage`. Inside the class you need to create a constructor with `public function new()` where you define all of the objects you want to add to the stage. Don't forget to call `super()`.
