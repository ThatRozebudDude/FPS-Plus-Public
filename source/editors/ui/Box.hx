...
class Box extends FlxSliceSprite
{
	...
	var preventSignals:Bool = false; //Makes sure that the signals can only fire once per frame.
	...
	public function new(_x:Float, _y:Float, _width:Float, _height:Float){
		...
		FlxG.signals.postUpdate.add(updatePreventSignals);
	}
	...
	function updatePreventSignals():Void{
		preventSignals = false;
	}

	override function destroy(){
		FlxG.signals.postUpdate.remove(updatePreventSignals);
		super.destroy();
	}
